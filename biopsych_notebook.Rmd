---
title: "BioPsych 2022 Companion Notebook"
...
```{r}
# Delta learning rule (outcome = x_[i,t,B] in paper)
...
sim_behav <- foreach(t=1:n_trials, .combine = "rbind") %do% {
  ...
  outcome <- sample(c(0,1), 1, prob=c(1-win_pr[choice], win_pr[choice]))
...
choice_value <- sim_behav %>%
  ...
  geom_hline(yintercept = .8, linetype = 2, color = reds[2]) +
  ...
  geom_text(x = 40, y = .91, label="Option 1", color = reds[2]) +
  geom_text(x = 60, y = .4, label="Option 2", color = "darkgray") +
  xlab("Trial") +
...
choice_prob <- sim_behav %>%
  ...
  geom_text(x = 40, y = .67, label="Option 1", color = reds[2]) +
  geom_text(x = 60, y = .32, label="Option 2", color = "darkgray") +
  scale_x_continuous(breaks = seq(0,100,10)) +
...
sim_error_generative <- foreach(i=1:n_iters, .combine = "rbind") %do% {
  foreach(sig=sigma_neural, .combine = "rbind") %do% {
    ...
    stan_dat$y_neural <- matrix(
      rnorm(length(ts_measure), (sim_behav$E)%*%design_matrix, sig), 
      nrow = 1
    )
...
error_plot_generative + 
  geom_line(aes(x = sigma_neural, y = R), data = summary_posthoc, color = "darkgray") +
  geom_ribbon(aes(ymin = R_lower, ymax = R_upper), data = summary_posthoc, alpha = .5, fill = "gray") +
  geom_text(x = .8, y = .7, label="Generative", color = reds[1], size = 5) +
  geom_text(x = .3, y = 0, label="Post-hoc", color = "darkgray", size = 5)
```
